...
/**
 * @author Zhinng  2013-04-01
 **/


/*
 * 角色: VISITOR < USER < ADMIN < SUPER_ADMIN
 **/

define('SP_ANONYMOUS','SP_ANONYMOUS'); // 无权限设置的角色名称
define('SP_USER', 'SP_USER');	// 普通用户
define('SP_ADMIN', 'SP_ADMIN');	// 管理员
define('SP_SUPER_ADMIN', 'SP_SUPER_ADMIN');	// 超级管理员


/**
 * 基于组的用户权限判断机制
 * 要使用该权限控制程序，需要在应用程序配置中做以下配置：
 * 有限控制的情况，在配置中使用	'launch' => array( 'router_prefilter' => array( array('spAclExtend','mincheck'), ), )
 * 强制控制的情况，在配置中使用	'launch' => array( 'router_prefilter' => array( array('spAclExtend','maxcheck'), ), )
 */
class spAclExtend
{
	/**
	 ...
	 */
	public $prompt = array('spAclExtend','def_prompt');
	/**
	 * 构造函数，设置权限检查程序与提示程序
	 */
	public function __construct()
	{	
		$params = spExt("spAclExtend");
...
class spAclModel extends spModel
{
	 ...
	 * @param action    动作名称
	 */
	public function check($acl_name = SP_ANONYMOUS, $controller, $action)
	{
		$rows = array('controller' => $controller, 'action' => $action );
		if( $acl = $this->findAll($rows) ){
			foreach($acl as $v){
				if($v["acl_name"] == SP_ANONYMOUS || $v["acl_name"] == $acl_name)return 1;
			}
			return 0;
		}else{
			return -1;
